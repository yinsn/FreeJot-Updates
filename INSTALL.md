...
供安装 agent 执行。核验日期：2026-09-26。系统要求：Apple Silicon（M 系列）Mac、macOS 14 或更新版本。首次准备至少留出 20 GB 可用空间；应用和两个模型约占 6.4 GB。运行和离线安装只需 Mac 自带工具，无需 Python、Xcode 或 API Key。

**直接安装包含全部模型的完整离线版 0.2.4，无需 GitHub 账号，也无需先装旧版。** 按下方清单下载安装助手和全部三个分卷，合计约 5.37 GB。约 27 MB 的 `FreeJot-Update-*.zip` 用于已有完整离线版的升级，不能单独完成首次安装。完整包已带齐 tokenizer 和两个模型。安装后按第 6 节在应用内更新到当前版本 **0.2.24**。
...
校验器检查应用标识、架构、离线标记、签名、模型清单及全部 20 个模型文件的 SHA256，包括 **4,760,339 字节的语音分词器 `tokenizer.json`**。这套模型校验适用于完整 0.2.0 及复用同一组模型的 0.2.1–0.2.24；未来更换模型时应使用对应发布的配套清单。

新装完整包应显示版本 `0.2.4`、构建号 `20260914-103027`。只有校验成功才继续；文件校验不代表麦克风、快捷键和自动输入已验证。

## 6. 安装后检查更新；旧版用户如何升级

**刚装好完整 0.2.4，或原来已装 0.2.2 及以上版本：**打开「设置 → 关于释手 → 检查更新」，下载并更新到 **0.2.24 / 20260926-111646（UTC）**。0.2.8 修复了更新交接时短暂进程读取失败会立即中止的问题。更新复用本机模型；完成后进入第 7 节权限与实际验收。若已是同版本或更新版本，无需重复下载安装。

**原来已装完整 0.2.0 / 0.2.1：**这些旧版没有应用内更新入口，按下面步骤手动升级一次，可直接到 0.2.24，无需逐版安装。
...
2. 当前已核验包为 [FreeJot-Update-0.2.24-AppleSilicon.zip](https://github.com/yinsn/FreeJot-Updates/releases/download/v0.2.24/FreeJot-Update-0.2.24-AppleSilicon.zip)，大小 `26938428` 字节，SHA256 为 `ed2792df1802ff5902031e3bf71f79f604b1875c9bb9245c112b9cc0405e1700`。未来 feed 版本更高时，使用那次发布的元数据，不沿用这里的摘要。
...
在设置里选择实际可用的快捷键。默认是 F16；MacBook 键盘没有 F16 时可选右 Command，将右 Option 留给英语听写。等语音识别模型就绪后，在备忘录等可编辑文本区放好光标，按一次开始录音，说一句话，再按一次结束，确认文字出现并自动输入。不要一直按住按键。0.2.4 起，对未保存过的设置默认选择「规则整理」，开启「保护第一个字」和「始终播放丝带动画」。使用有线或内置麦克风时，保护开启且获得麦克风权限后，麦克风保持就绪，空闲时只在内存保留最近约 300 毫秒声音；系统麦克风指示会持续显示。用户明确保存过的模式或开关选择优先于新版默认值。
...
0.2.24 缩短短句识别等待：录音期间每约 2 秒准备一份草稿，停止后用完整录音核对，允许纠正前文和句尾；缩小音频回调请求，最后一段音频到齐后立即继续处理。普通听写与右 Option 英语听写均生效，无需额外设置。沿用现有模型、精度和长录音分段方案；实际收益取决于句长、机器负载和音频设备。

0.2.23 提速长录音：录音达到约 33 秒后，开始提前识别切分位置已稳定的前段；结束录音时复用已完成的识别，只处理剩余内容，再对全文进行一次整理或英语转换，并一次性输入。沿用原有模型、精度与分段规则；短句沿用原有识别流程，右 Option 英语入口和 UU 远程粘贴等待保留。

0.2.22 将英语听写快捷键改为仅右侧 Option：单独按下并松开开始，再按一下结束，左 Option 不触发英语翻译。普通听写快捷键与已保存的英语开关保留；若普通听写已占用右 Option，则提示更换普通快捷键，英语入口不再改用左侧。

0.2.21 加快简体中文的智能整理：批量验证与原文相同的文字，减少逐字计算，仍由原模型逐个确认输出。纯英文、繁体中文和过短文本沿用原有路径。模型、精度、提示词、词典和输出保护保持不变；语音识别阶段与规则整理不变。
...
0.2.18 新增英语听写，将中文、英文或中英混合整理成标准英语。0.2.22 起仅单按右 Option 开始、再按一下结束；组合键和配合鼠标点击不触发。原普通快捷键与整理模式保留。可在「设置 → 快捷键」关闭。英语转换复用本机模型，规则模式临时加载，用完释放；失败保留原文并提示，不粘贴中文回退。
