...
**直接安装包含全部模型的完整离线版 0.2.4，无需 GitHub 账号，也无需先装旧版。** 按下方清单下载安装助手和全部三个分卷，合计约 5.37 GB。约 27 MB 的 `FreeJot-Update-*.zip` 用于已有完整离线版的升级，不能单独完成首次安装。完整包已带齐 tokenizer 和两个模型。安装后按第 6 节在应用内更新到当前版本 **0.2.5**。
...
校验器检查应用标识、架构、离线标记、签名、模型清单及全部 20 个模型文件的 SHA256，包括 **4,760,339 字节的语音分词器 `tokenizer.json`**。这套模型校验适用于完整 0.2.0 及复用同一组模型的 0.2.1–0.2.5；未来更换模型时应使用对应发布的配套清单。

新装完整包应显示版本 `0.2.4`、构建号 `20260914-103027`。只有校验成功才继续；文件校验不代表麦克风、快捷键和自动输入已验证。

## 6. 安装后检查更新；旧版用户如何升级

**刚装好完整 0.2.4，或原来已装 0.2.2 及以上版本：**打开「设置 → 关于释手 → 检查更新」，下载并更新到 **0.2.5 / 20260914-163244（UTC）**。更新复用本机模型；完成后进入第 7 节权限与实际验收。若已是同版本或更新版本，无需重复下载安装。

**原来已装完整 0.2.0 / 0.2.1：**这些旧版没有应用内更新入口，按下面步骤手动升级一次，可直接到 0.2.5，无需逐版安装。
...
2. 当前已核验包为 [FreeJot-Update-0.2.5-AppleSilicon.zip](https://github.com/yinsn/FreeJot-Updates/releases/download/v0.2.5/FreeJot-Update-0.2.5-AppleSilicon.zip)，大小 `26701325` 字节，SHA256 为 `c8893ff7e3eee42b9c6f60785a7720ff34bdc549be9b26e554615d2944abf946`。未来 feed 版本更高时，使用那次发布的元数据，不沿用这里的摘要。
...
0.2.5 可在「设置 → 语音识别 → 录音麦克风」为释手单独选择输入，每台 Mac 各自保存。自动选择优先使用 USB 等外接话筒，再使用内置麦克风，不自动选择蓝牙输入。DJI Mic Mini 通过 USB 接收器连接时可自动选择并使用开头保护；蓝牙直连且被 macOS 识别为音频输入时，需要手动选定，开头保护自动暂停，只在录音期间打开输入，等录音提示出现后再说话。指定设备断开时提示重新连接，不切换到其他话筒。会议软件需要自行选择独立话筒；释手不会修改系统或会议软件的输入输出设置。
