...
供安装 agent 执行。核验日期：2026-09-17。系统要求：Apple Silicon（M 系列）Mac、macOS 14 或更新版本。首次准备至少留出 20 GB 可用空间；应用和两个模型约占 6.4 GB。运行和离线安装只需 Mac 自带工具，无需 Python、Xcode 或 API Key。

**直接安装包含全部模型的完整离线版 0.2.4，无需 GitHub 账号，也无需先装旧版。** 按下方清单下载安装助手和全部三个分卷，合计约 5.37 GB。约 27 MB 的 `FreeJot-Update-*.zip` 用于已有完整离线版的升级，不能单独完成首次安装。完整包已带齐 tokenizer 和两个模型。安装后按第 6 节在应用内更新到当前版本 **0.2.14**。
...
校验器检查应用标识、架构、离线标记、签名、模型清单及全部 20 个模型文件的 SHA256，包括 **4,760,339 字节的语音分词器 `tokenizer.json`**。这套模型校验适用于完整 0.2.0 及复用同一组模型的 0.2.1–0.2.14；未来更换模型时应使用对应发布的配套清单。

新装完整包应显示版本 `0.2.4`、构建号 `20260914-103027`。只有校验成功才继续；文件校验不代表麦克风、快捷键和自动输入已验证。

## 6. 安装后检查更新；旧版用户如何升级

**刚装好完整 0.2.4，或原来已装 0.2.2 及以上版本：**打开「设置 → 关于释手 → 检查更新」，下载并更新到 **0.2.14 / 20260917-113610（UTC）**。0.2.8 修复了更新交接时短暂进程读取失败会立即中止的问题。更新复用本机模型；完成后进入第 7 节权限与实际验收。若已是同版本或更新版本，无需重复下载安装。

**原来已装完整 0.2.0 / 0.2.1：**这些旧版没有应用内更新入口，按下面步骤手动升级一次，可直接到 0.2.14，无需逐版安装。
...
2. 当前已核验包为 [FreeJot-Update-0.2.14-AppleSilicon.zip](https://github.com/yinsn/FreeJot-Updates/releases/download/v0.2.14/FreeJot-Update-0.2.14-AppleSilicon.zip)，大小 `26795911` 字节，SHA256 为 `e05243d0ca6b7026a248bdbf2e29a96695e82ceed4d99c18965a74f58af0f353`。未来 feed 版本更高时，使用那次发布的元数据，不沿用这里的摘要。
...
0.2.7 可在「设置 → 语音识别 → 录音麦克风」为释手单独选择输入，每台 Mac 各自保存。自动选择优先使用 USB 等外接话筒，再使用内置麦克风，不自动选择蓝牙输入。DJI Mic Mini 通过 USB 接收器连接时可自动选择；蓝牙直连且被 macOS 识别为音频输入时，需要手动选定。蓝牙话筒现在也遵循「保护第一个字」开关：开启时持续保持输入就绪并保留最近约 300 毫秒；关闭时按键后打开输入，结束或取消后释放。指定设备断开时提示重新连接，不切换到其他话筒。若选择耳机自带麦克风，持续占用输入可能影响该耳机的播放音质。

0.2.7 新增「语音识别 → 系统麦克风 → 运行时固定系统麦克风」，默认关闭。需要防止系统自动改用耳机话筒的机器，应先在上方选定本机的独立话筒，再开启此开关；确认显示「已固定系统输入」和正确设备名。以后启动释手会自动恢复固定，所选话筒断开时暂停，重连后继续。关闭功能或正常退出时，仅当当前输入仍是释手最后设置的设备且原输入仍可用，才恢复开启前的系统输入；强制结束进程无法恢复。0.2.9 在输入切换前保留播放与提示音设备，纠正切换造成的输出偏移；蓝牙过渡期间若输出切到所选话筒，会尝试恢复原耳机。恢复输出导致输入再次改变时暂停固定，避免反复抢切。新增「声音播放」选择，若之前已无声，先在此选回 AirPods 或实际耳机，核对「当前声音播放」设备名。用户切换到其他耳机或扬声器时保留新选择，不修改音量、设备采样率或会议软件各自保存的输入；会议软件若明确选择了 AirPods 麦克风，需在该软件中改选。它不能全局禁用耳机话筒。

0.2.10 改进英文纠偏：开启「文字整理 → 智能整理」后，会结合个人词典和句意修正英文误识、拼写与拆词。常用专名可加入词典；词典不直接参与语音识别。所有模式最终都会补齐中英文之间的空格，包括规则整理、跳过整理和模型回退，例如 `用Astra检查API` 输出为 `用 Astra 检查 API`。

0.2.12 的数字输出不依赖智能整理：“一个问题、两个人、三次、两天、十分钟”等一到十的日常小数量保留中文；识别或整理已经输出 `1 个`、`2 个人` 时也会还原，并清掉原数字两侧的多余空格。日期、时刻、金额、度量、版本、编号、章节页码和明确统计数值仍用阿拉伯数字：`两百元` → `200 元`、`数量是两个` → `数量是 2 个`、`百分之十二点五` → `12.5%`、`F 十六` → `F16`。较大数量仍用数字。“一起”“万一”“每一个”“三心二意”等固定表达、星期名称与明确引用的汉字保留。“三五天”“十几个”“一百二”等含糊数量保留原说法。

0.2.14 加强确认问句保护：智能整理必须保留“是吧、对吧、好吗、好么”和“吗、吧”等表达；若模型删除、增加或替换受保护表达，则回退到原文并执行基础清理。校验不依赖逗号或问号，也检查重复出现的确认表达。所有模式最终仍去掉句末标点，并用正则将非空文本的末尾统一为一个普通空格；空结果不输出空格，重复处理不会累积空格。

0.2.13 修复部分离散声道输入被转换为静音的问题。「语音识别 → 录音诊断」会显示最近录音的输入声道数和峰值；峰值为 0 时检查所选输入、静音状态和系统输入音量。该转换缺陷已用合成音频验证；MacBook Air 内置麦克风需在目标机器上实际验证，不能由安装包校验推断已修复。

应用更新保留目标 Mac 已保存的设置，不携带开发机器的设备选择或开关值。在另一台机器使用「独立蓝牙话筒输入＋AirPods 播放」时，需要实际确认录音、开头完整、播放稳定、断线提示和重连恢复。自动化设备模拟、安装包校验及一台 Mac 上的验证不能代替这个组合在目标机器上的实际验收。
...
- 麦克风、辅助功能、输入监视权限已由用户确认；选定输入在保护开启时显示「已就绪」；关闭保护时需等录音提示出现再说话。
- 若开启系统麦克风固定，设备名正确且显示「已固定系统输入」；断线与恢复行为符合上文说明。
- 在目标 Mac 上实际录音、转写和自动输入成功；若同时使用蓝牙话筒和蓝牙耳机，还应确认开头完整及耳机播放稳定。同一时刻仅运行一份应用。
...
| 文件校验通过但没有声音 | 检查麦克风权限及「录音麦克风」所选设备是否连接；开启开头保护时等「已就绪」；关闭保护时等录音提示出现再说话 |
