...
**直接安装包含全部模型的完整离线版 0.2.4，无需 GitHub 账号，也无需先装旧版。** 按下方清单下载安装助手和全部三个分卷，合计约 5.37 GB。约 27 MB 的 `FreeJot-Update-*.zip` 用于已有完整离线版的升级，不能单独完成首次安装。完整包已带齐 tokenizer 和两个模型。安装后按第 6 节在应用内更新到当前版本 **0.2.18**。
...
校验器检查应用标识、架构、离线标记、签名、模型清单及全部 20 个模型文件的 SHA256，包括 **4,760,339 字节的语音分词器 `tokenizer.json`**。这套模型校验适用于完整 0.2.0 及复用同一组模型的 0.2.1–0.2.18；未来更换模型时应使用对应发布的配套清单。

新装完整包应显示版本 `0.2.4`、构建号 `20260914-103027`。只有校验成功才继续；文件校验不代表麦克风、快捷键和自动输入已验证。

## 6. 安装后检查更新；旧版用户如何升级

**刚装好完整 0.2.4，或原来已装 0.2.2 及以上版本：**打开「设置 → 关于释手 → 检查更新」，下载并更新到 **0.2.18 / 20260922-100130（UTC）**。0.2.8 修复了更新交接时短暂进程读取失败会立即中止的问题。更新复用本机模型；完成后进入第 7 节权限与实际验收。若已是同版本或更新版本，无需重复下载安装。

**原来已装完整 0.2.0 / 0.2.1：**这些旧版没有应用内更新入口，按下面步骤手动升级一次，可直接到 0.2.18，无需逐版安装。
...
2. 当前已核验包为 [FreeJot-Update-0.2.18-AppleSilicon.zip](https://github.com/yinsn/FreeJot-Updates/releases/download/v0.2.18/FreeJot-Update-0.2.18-AppleSilicon.zip)，大小 `26837410` 字节，SHA256 为 `d4459d130c559ad787afd5ad3ccb6dc62357672a1cd1c899bb9d825abdca97c4`。未来 feed 版本更高时，使用那次发布的元数据，不沿用这里的摘要。
