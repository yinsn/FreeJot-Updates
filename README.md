...
先读 [完整安装指南](https://yinsn.github.io/FreeJot-Updates/INSTALL.md) 或 [网页版](https://yinsn.github.io/FreeJot-Updates/install.html)。完整 0.2.4 安装包包含程序和两个模型，约 5.37 GB。直接从首页下载安装助手和全部三个分卷，按指南校验、合并和安装，不必先装旧版。四个附件均在本公开仓库的 v0.2.4 Release，可直接下载，无需登录或源码仓库权限。安装后在应用内检查更新到当前版本 **0.2.6**。
...
使用公开的轻量更新包，复用本机模型。0.2.1 及更早版本需手动升级一次；完整 0.2.0 可直接升级到 0.2.6，无需逐版安装。0.2.2 及以后可在「设置 → 关于释手 → 检查更新」完成后续升级。
